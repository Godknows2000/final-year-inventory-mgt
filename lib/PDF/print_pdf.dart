...
class GeneratePdfAndPrint {
  Future<void> uploadPdfToFirebase(
      Uint8List pdfData, String fileType, String invoiceNumber) async {
  ...
  Future<void> printSaleReturnInvoice(
      ...
      bool? fromInventorySale}) async {
    EasyLoading.show(status: 'Generating PDF...', dismissOnTap: true);
    var pdfData = await generateSaleReturnDocument(
        personalInformation: personalInformationModel,
        transactions: saleTransactionModel);
    await uploadPdfToFirebase(
        pdfData, 'sale-return', saleTransactionModel.invoiceNumber);
    EasyLoading.dismiss();
    await Printing.layoutPdf(
      dynamicLayout: true,
      onLayout: (PdfPageFormat format) async => pdfData,
  ...
  Future<void> printQuotationInvoice(
    ...
    EasyLoading.show(status: 'Generating PDF...', dismissOnTap: true);
    var pdfData = await GeneratePdfAndPrint().generateQuotationDocument(
        personalInformation: personalInformationModel,
        transactions: saleTransactionModel);
    await uploadPdfToFirebase(
        pdfData, 'quotation', saleTransactionModel.invoiceNumber);
    EasyLoading.dismiss();
    await Printing.layoutPdf(
      dynamicLayout: true,
      onLayout: (PdfPageFormat format) async => pdfData,
  ...
  Future<void> printPurchaseInvoice(
    ...
    EasyLoading.show(status: 'Generating PDF...', dismissOnTap: true);
    var pdfData = await generatePurchaseDocument(
        personalInformation: personalInformationModel,
        transactions: purchaseTransactionModel);
    await uploadPdfToFirebase(
        pdfData, 'purchase', purchaseTransactionModel.invoiceNumber);
    EasyLoading.dismiss();
    await Printing.layoutPdf(
      dynamicLayout: true,
      onLayout: (PdfPageFormat format) async => pdfData,
  ...
  Future<void> printPurchaseReturnInvoice(
      ...
      BuildContext? context}) async {
    EasyLoading.show(status: 'Generating PDF...', dismissOnTap: true);
    var pdfData = await generatePurchaseReturnDocument(
        personalInformation: personalInformationModel,
        transactions: purchaseTransactionModel);
    await uploadPdfToFirebase(
        pdfData, 'purchase-return', purchaseTransactionModel.invoiceNumber);
    EasyLoading.dismiss();
    await Printing.layoutPdf(
      dynamicLayout: true,
      onLayout: (PdfPageFormat format) async => pdfData,
  ...
  Future<void> printDueInvoice(
      ...
      BuildContext? context}) async {
    EasyLoading.show(status: 'Generating PDF...', dismissOnTap: true);
    var pdfData = await GeneratePdfAndPrint().generateDueDocument(
        personalInformation: personalInformationModel,
        transactions: dueTransactionModel);
    await uploadPdfToFirebase(
        pdfData, 'due', dueTransactionModel.invoiceNumber);
    EasyLoading.dismiss();
    await Printing.layoutPdf(
      dynamicLayout: true,
      onLayout: (PdfPageFormat format) async => pdfData,
        ...
        footer: (pw.Context context) {
          return pw.Column(children: [
            ...
            pw.Container(
              ...
              padding: const pw.EdgeInsets.all(10.0),
              child: pw.Center(
                  child: pw.Text('Final Year Project',
                      style: pw.TextStyle(
